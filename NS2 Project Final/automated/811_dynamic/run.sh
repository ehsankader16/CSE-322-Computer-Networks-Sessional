...
tcl_file_to_run="wireless_811_dynamic.tcl"
...
do
    ...
    echo testing "$tcl_file_to_run" $unmodified $i $flow_default $packet_default $speed_default $coverage_default
    ns "$tcl_file_to_run" $unmodified $i $flow_default $packet_default $speed_default $coverage_default
    awk -f parse.awk trace.tr >> $node_result_file
    echo testing "$tcl_file_to_run" $modified $i $flow_default $packet_default $speed_default $coverage_default
    ns "$tcl_file_to_run" $modified $i $flow_default $packet_default $speed_default $coverage_default
...
do
    ...
    echo testing "$tcl_file_to_run" $unmodified $node_default $i $packet_default $speed_default $coverage_default
    ns "$tcl_file_to_run" $unmodified $node_default $i $packet_default $speed_default $coverage_default
    awk -f parse.awk trace.tr >> $flow_result_file
    echo testing "$tcl_file_to_run" $modified $node_default $i $packet_default $speed_default $coverage_default
    ns "$tcl_file_to_run" $modified $node_default $i $packet_default $speed_default $coverage_default
...
do
    ...
    echo testing "$tcl_file_to_run" $unmodified $node_default $flow_default $packet_default $i $coverage_default
    ns "$tcl_file_to_run" $unmodified $node_default $flow_default $packet_default $i $coverage_default
    awk -f parse.awk trace.tr >> $speed_result_file
    echo testing "$tcl_file_to_run" $modified $node_default $flow_default $packet_default $i $coverage_default
    ns "$tcl_file_to_run" $modified $node_default $flow_default $packet_default $i $coverage_default
...
do
    ...
    echo testing "$tcl_file_to_run" $unmodified $node_default $flow_default $packet_default $speed_default $i
    ns "$tcl_file_to_run" $unmodified $node_default $flow_default $packet_default $speed_default $i
    awk -f parse.awk trace.tr >> $coverage_result_file
    echo testing "$tcl_file_to_run" $modified $node_default $flow_default $packet_default $speed_default $i
    ns "$tcl_file_to_run" $modified $node_default $flow_default $packet_default $speed_default $i
